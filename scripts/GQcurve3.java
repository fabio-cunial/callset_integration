 ...
 * 
 */
public class GQcurve3 {
    
    private static final char COMMENT = '#';
    private static final String GT_SEPARATOR = ":";
    private static final String STDEV_STR = "STDEV_LEN";
    ...
    public static void main(String[] args) throws IOException {
        ...
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/roc_calls_"+STDEV_STR+"_"+(GEQ?"geq":"leq")+".txt"));
        for (i=0; i<=max; i++) {
            truePositives=nTP[i];
        ...
        bw.close();
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/roc_del_"+STDEV_STR+"_"+(GEQ?"geq":"leq")+".txt"));
        for (i=0; i<=max; i++) {
        ...
        bw.close();
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/roc_ins_"+STDEV_STR+"_"+(GEQ?"geq":"leq")+".txt"));
        for (i=0; i<=max; i++) {
        ...
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/histogram_calls_"+STDEV_STR+".txt"));
        for (i=0; i<=max; i++) bw.write(i+","+nCalls_histogram[i]+"\n");
        bw.close();
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/histogram_del_"+STDEV_STR+".txt"));
        for (i=0; i<=max; i++) bw.write(i+","+nDel_histogram[i]+"\n");
        bw.close();
        bw = new BufferedWriter(new FileWriter(OUTPUT_DIR+"/histogram_ins_"+STDEV_STR+".txt"));
    ...
    private static final int getMax(String vcfFile) throws IOException {
        ...
        while (str!=null) {
            if (str.charAt(0)==COMMENT) { str=br.readLine(); continue; }
            tokens=str.split("\t");
            field=VCFconstants.getField(tokens[7],STDEV_STR);
