...
CALLER_VCF="${ROOT_DIR}/HG002.truvari_collapsed.vcf.gz"
ALIGNMENTS_BAM="${ROOT_DIR}/HG002.bam"
READS_FASTQ_GZ="${ROOT_DIR}/HG002.fastq.gz"
TRUTH_VCF="${ROOT_DIR}/GRCh38_HG002-T2TQ100-V0.9_stvar.vcf.gz"
TRUTH_BED="${ROOT_DIR}/GRCh38_HG002-T2TQ100-V0.9_stvar.benchmark.bed"
REFERENCE_FA="${ROOT_DIR}/GCA_000001405.15_GRCh38_no_alt_analysis_set.fa"
TR_BED="${ROOT_DIR}/GRCh38_AllTandemRepeatsandHomopolymers_slop5.bed"
NONTR_BED="${ROOT_DIR}/GRCh38_notinAllTandemRepeatsandHomopolymers_slop5.bed"

TR_SMALL_OVERLAP="0.1"
TR_LARGE_OVERLAP="0.9"
SVLEN_BINS="100,500,2500,5000,10000,20000,50000";
SVLEN_MIN="50"; SVLEN_MAX="50000";
...
function formatVcf() {
    ...
    ${RESOLVE_COMMAND} ${INPUT_VCF_GZ} ${REFERENCE_FA} | bcftools view -i "SVTYPE!='BND'" | bcftools sort -O z -o ${ROOT_DIR}/tmp1.vcf.gz
...
function afterRegenotyping() {
    ...
    N_CALLS_00_AFTER=$(bcftools filter -i "${FILTER_STRING}" ${ROOT_DIR}/regenotyped.vcf.gz | grep '^[^#]' | wc -l)
    ...
    if [ ${GENOTYPER} = sniffles -o ${GENOTYPER} = cutesv ]; then
        ...
        java ROCcurve SQ 1 ${SVLEN_MIN} ${SVLEN_MAX} ${ROOT_DIR}/regenotyped.vcf ${ROOT_DIR}/truvari/tp-comp.vcf ${ROOT_DIR}/truth.vcf ${ROOT_DIR} ${TR_STATUS} ${GENOTYPER} ${SVLEN_BINS}
...
for TR_STATUS in 0 1 2 3; do
    ...
    truvari anno svinfo --minsize 0 ${TRUTH_VCF} | bgzip > ${ROOT_DIR}/tmp0.vcf.gz
    tabix ${ROOT_DIR}/tmp0.vcf.gz
    formatVcf ${ROOT_DIR}/tmp0.vcf.gz ${ROOT_DIR}/truth.vcf.gz 1 ${TR_STATUS}
    ...
    N_CALLS_00_BEFORE=$(bcftools filter -i "${FILTER_STRING}" ${ROOT_DIR}/merged.vcf.gz | grep '^[^#]' | wc -l)
