...
public class GQcurve6 {
    ...
    private static final int getMax(String vcfFile) throws IOException {
        ...
        while (str!=null) {
            if (str.charAt(0)==COMMENT) { str=br.readLine(); continue; }
            tokens=str.split("\t");
            field=VCFconstants.getField(tokens[7],CIPOS_STR);
            if (field==null) { str=br.readLine(); continue; }
            p=field.indexOf(CIPOS_SEPARATOR);
            value=Math.abs(Double.parseDouble(field.substring(0,p)))+Math.abs(Double.parseDouble(field.substring(p+1)));
