...
task GetRegenotypedVcfImpl {
    ...
    command <<<
        ...
        ~{docker_dir}/kanpig --threads ${N_THREADS} --debug --sizemin ~{svlen_min} --sizemax ~{svlen_max} ${KANPIG_PARAMS} --input merged.vcf.gz --bam ~{alignments_bam} --reference ~{reference_fa} --out tmp1.vcf.gz
