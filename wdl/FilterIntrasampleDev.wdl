...
# Sniffles: ['SQ', 'GQ', 'DR', 'DV', 'SUPP_PAV', 'SUPP_SNIFFLES', 'SUPP_PBSV']
# kanpig: ['SQ', 'GQ', 'DP', 'AD_NON_ALT', 'AD_ALL', 'SUPP_PAV', 'SUPP_SNIFFLES', 'SUPP_PBSV']
...
task PreprocessVCF {
    ...
    command <<<
        ...
        if ~{is_sniffles}; then
            ...
            # SQ, GQ, DP, AD from kanpig-regenotyped VCF
...
EOF
        ...
        if ~{is_sniffles}; then
            # GQ, DR, DV from Sniffles-regenotyped VCF
            bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t[%GQ]\t[%DR]\t[%DV]\n' ~{regenotyped_vcf_gz} | bgzip -c > format.tsv.gz
            tabix -s1 -b2 -e2 format.tsv.gz
            bcftools annotate -a format.tsv.gz -h format.hdr.txt -c CHROM,POS,REF,ALT,GQ,DR,DV format.supp_binary.vcf.gz -Oz -o ~{output_prefix}.preprocessed.vcf.gz
        else
            # SQ, GQ, DP, AD from kanpig-regenotyped VCF; TODO nicer AD
            bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t[%SQ]\t[%GQ]\t[%DP]\t[%AD]\n' ~{regenotyped_vcf_gz} | sed -E 's/,/\t/g' | bgzip -c > format.tsv.gz
            tabix -s1 -b2 -e2 format.tsv.gz
            bcftools annotate -a format.tsv.gz -h format.hdr.txt -c CHROM,POS,REF,ALT,SQ,GQ,DP,AD_NON_ALT,AD_ALL format.supp_binary.vcf.gz -Oz -o ~{output_prefix}.preprocessed.vcf.gz
        fi
