...
#
workflow GetRegenotypedVcfKanpigV11Kanpig102 {
    input {
...
task GetRegenotypedVcfImpl {
    ...
    command <<<
        ...
        ~{docker_dir}/kanpig gt --threads $(( ${N_THREADS} - 1)) --ploidy-bed ${PLOIDY_BED} ~{kanpig_params_singlesample} --reference ~{reference_fa} --input cleaned.vcf.gz --reads ~{alignments_bam} --out tmp1.vcf.gz
